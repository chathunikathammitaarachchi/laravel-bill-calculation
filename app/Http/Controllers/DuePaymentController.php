 ...
 public function payDueByCustomer(Request $request)
{
    ...
    DB::transaction(function () use ($request, &$paymentDetails, &$dues) {
    ...
    $paymentDetails['totalDue'] = $dues->sum('tobe_price');
    $paymentDetails['totalPaid'] = $dues->sum('customer_pay');
    $paymentDetails['totalBalance'] = $dues->sum('balance');

    //  Instead of PDF or redirect, just return receipt view directly
return view('receipt.customer_due_payment_printable', $paymentDetails);
}
